...
const getSocialItems = (t) => [
  ...
  },
  {
    ...
    label: t.social.wechatId,
    qr: '/images/qr/wechat-qr.png',
    qrAlt: t.social.wechatQrAlt,
    icon: (
  ...
  },
  {
    ...
    label: t.social.instagramHandle,
    qr: '/images/qr/ig-qr.png',
    qrAlt: t.social.instagramQrAlt,
    icon: (
  ...
  },
  {
    ...
    label: t.about.xiaohongshuChannel,
    qr: '/images/qr/redbook-qr.png',
    qrPosition: 'bottom',
    qrAlt: t.social.xiaohongshuQrAlt,
    icon: (
      <img
...
const VARIANT_STYLES = {
  dark: {
    ...
    popupClassBottom:
      'pointer-events-none absolute top-full left-0 z-50 mt-3 opacity-0 transition-opacity duration-200 group-hover:opacity-100',
  ...
  light: {
    ...
    popupClassBottom:
      'pointer-events-none absolute top-full left-0 z-10 mt-3 opacity-0 transition-opacity duration-200 group-hover:opacity-100',
    qrImageClass: 'w-40 rounded-xl border border-stone-200 shadow-xl',
  },
  soft: {
    itemClass:
      'group relative flex items-center gap-3 rounded-xl border border-stone-300 bg-white px-5 py-3.5 text-sm font-semibold text-stone-800 transition hover:bg-stone-50',
    popupClassTop:
      'pointer-events-none absolute bottom-full left-0 z-10 mb-3 opacity-0 transition-opacity duration-200 group-hover:opacity-100',
    popupClassBottom:
      'pointer-events-none absolute top-full left-0 z-10 mt-3 opacity-0 transition-opacity duration-200 group-hover:opacity-100',
 ...
 */
export default function SocialLinks({ variant = 'dark', excludeKeys = [] }) {
  const { t } = useLanguage();
  const SOCIAL_ITEMS = getSocialItems(t);
  const excludedKeys = new Set(Array.isArray(excludeKeys) ? excludeKeys : []);
  const styles = VARIANT_STYLES[variant] ?? VARIANT_STYLES.dark;
  const visibleItems = SOCIAL_ITEMS.filter((item) => !excludedKeys.has(item.key));
  ...
  return (
    <>
      {visibleItems.map((item) => {
        ...
        const popupClass = item.qrPosition === 'bottom' ? styles.popupClassBottom : styles.popupClassTop;
